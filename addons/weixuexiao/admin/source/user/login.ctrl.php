 ...
 *
 * */
$filename = IA_ROOT.'/addons/weixuexiao/inc/web/common.func.php'; //当前目录文件
...
function _login($forward = '') {
    ...
    if(!empty($record)) {
		...
		if(IMS_RELEASE_DATE >= '201709120001' && IMS_RELEASE_DATE < '201903070005'){
            ...
            //switch_save_module_display($uniacid, 'weixuexiao');
		}
		...
		if(is_TestFz()){
			...
			if(!empty($HasQxLogin)){
				$furl = $_W['siteroot'].'web/index.php?c=site&a=entry&uid='.$record['uid'].'&from=depend&do=newtype&m=weixuexiao';
			}else{
				$furl = $_W['siteroot'].'web/index.php?c=site&a=entry&uid='.$record['uid'].'&from=depend&do=start&id='.$schoolid.'&i='.$uniacid.'&schoolid='.$schoolid.'&m=weixuexiao';
			}
		}else{
			$furl = $_W['siteroot'].'web/index.php?c=site&a=entry&uid='.$record['uid'].'&from=depend&do=start&id='.$schoolid.'&i='.$uniacid.'&schoolid='.$schoolid.'&m=weixuexiao';
		}
