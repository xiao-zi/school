...
class Mloader
{
	...
	function func($name)
	{
		...
		$file = IA_ROOT . '/addons/weixuexiao/function/' . $name . '.func.php';
		if (file_exists($file)) {
			include $file;
			$this->cache['func'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid Helper Function /addons/weixuexiao/function/' . $name . '.func.php', E_USER_ERROR);
			return false;
		}
	}
	function model($name)
	{
		...
		$file = IA_ROOT . '/addons/weixuexiao/model/' . $name . '.mod.php';
		if (file_exists($file)) {
			include $file;
			$this->cache['model'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid Model /addons/weixuexiao/model/' . $name . '.mod.php', E_USER_ERROR);
			return false;
		}
	}
	function classs($name)
	{
		...
		$file = IA_ROOT . '/addons/weixuexiao/class/' . $name . '.class.php';
		if (file_exists($file)) {
			include $file;
			$this->cache['class'][$name] = true;
			return true;
		} else {
			trigger_error('Invalid Class /addons/weixuexiao/class/' . $name . '.class.php', E_USER_ERROR);
			return false;
		}
	}
}
...
function sale(){
	mload()->model('kc');
	$result = check_plugin('weixuexiao_plugin_sale');
	return $result;
}
...
function vis(){
	...
	mload()->model('kc');
	$result = check_plugin('weixuexiao_plugin_vis');
	return $result;
}

function bigdata(){
	mload()->model('kc');
	$result = check_plugin('weixuexiao_plugin_bigdata');
	return $result;
}

function assets(){
	mload()->model('kc');
	$result = check_plugin('weixuexiao_plugin_assets');
	return $result;
}
...
function tpl_form_execl_input($schoolid,$type)
{
	...
	$exporturl = $_W['siteroot'] . 'web/index.php?c=site&a=entry&schoolid=' . $schoolid . '&do=chengji&op=export&m=weixuexiao';
	$inputurl = $_W['siteroot'] . 'web/index.php?c=site&a=entry&schoolid=' . $schoolid . '&do=chengji&op=input&m=weixuexiao';
	$s = '';
	$s .='
...
function upload_file_to_cdn($data,$host){
    $ch = curl_init();
	$url = 'http%3a%2f%2fauth.7mao.cc%2fapi%2famr.php';	
...
function opreatmac($macid,$mactype,$posturl,$type,$schoolname){
	...
	if(getoauthurl()){$url = 'http%3a%2f%2fauth.7mao.cc%2fapi%2fmac.php';}
	if(version_compare(PHP_VERSION,'5.5.0','>=')){
		curl_setopt($ch, CURLOPT_URL,urldecode($url));
		curl_setopt($ch, CURLOPT_POST, true);
		curl_setopt($ch, CURLOPT_HEADER, false);
...
function check_sales($sale){
	load()->func('communication');
	$url = 'http%3a%2f%2fauth.7mao.cc%2fapi%2fsaleapi.php';
	$data = array(
		'sale'   => $sale,
		'checkver'   => 'checkver',
		'oauthurl' => getoauthurl()
...
function visitors_qrcode($data = ''){
    include_once IA_ROOT . '/addons/weixuexiao/inc/web/phpqrcode.php';
    $errorCorrectionLevel = 'L';	//容错级别
    $matrixPointSize = 10;			//生成图片大小
    $qrcode_name = 'images/weixuexiao/'.time().'visitorqr'.rand(111111,999999).'.png';
 ...
 * 查询微擎会员信息
 */
function GetWeFans($weid,$openid){
	$data = array();
	$fans = pdo_fetch("SELECT fanid,uid,tag FROM " . tablename('mc_mapping_fans') . " where openid = '{$openid}' And uniacid = '{$weid}' ");
	if(!empty($fans)){
		if(empty($fans['tag'])){
		...
		$fansinfo = pdo_fetch("SELECT nickname,avatar as headimgurl FROM " . tablename('mc_members') . " where uid = '{$fans['uid']}' And uniacid = '{$weid}' ");
	}else{
		$account_api = WeAccount::create($weid);
...
function is_showap(){
   // return 1;
    global $_W;
    $oauthurl = getoauthurl();
    if( $oauthurl == "www.puerqirui.com" || $oauthurl == "www.anqinschool.com" ||$oauthurl == "manger.daren007.com" || $oauthurl =="www.hlwjjy.cn" )
        return 1;
    else
        return 0;
}
...
//     return 1;
    global $_W;
    $oauthurl = getoauthurl();
    if( $oauthurl == "edu.morree.com" || $oauthurl == 'manger.daren007.com')
        return 1;
    else
         return 0;
}
...
function anhui(){
    ...
    global $_W;
    $oauthurl = getoauthurl();
    if( $oauthurl == "anhui.daren007.com" || $oauthurl == "manger.daren007.com")
        return 1;
    else
        return 0;
}
...
function Keep_sk77(){
    ...
    global $_W;
    $oauthurl = getoauthurl();
    if( $oauthurl == "bly.sk77.cn" || $oauthurl == "manger.daren007.com")
        return 1;
    else
        return 0;
}
...
function keep_wt(){
    ...
    global $_W;
    $oauthurl = getoauthurl();
    if( $oauthurl == "vip.slok.com.cn" || $oauthurl == "manger.daren007.com")
        return 1;
    else
        return 0;
}
...
function keep_hxy(){
	global $_W;
    $oauthurl = getoauthurl();
	if(  $oauthurl == "manger.daren007.com" || $oauthurl == "edu.morree.com")
		return 1;
	else
		return 0;
