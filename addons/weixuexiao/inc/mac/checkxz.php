...
$operation = in_array ( $_GPC ['op'], array ('default', 'login', 'classinfo', 'check', 'gps', 'banner', 'video','checknew', 'checkold','all','banpai','allcard','getleave','onlineflow','busgps','offlineflow','cardinfo','checkforks','getallroom','syncmoney') ) ? $_GPC ['op'] : 'default';
$weid = $_GPC['i'];
...
if ($operation == 'login') {
    if(!empty($ckmac)){
        ...
        $result['data']['posturl']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=checknew&m=weixuexiao');
        $result['data']['oldposturl']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=checkold&m=weixuexiao');
        $result['data']['leaveurl']   = urlencode( $_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=getleave&m=weixuexiao');
        $result['data']['outTimeUrl']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=classinfo&m=weixuexiao');
        $result['data']['offlineflow']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=offlineflow&m=weixuexiao');
        $result['data']['syncmoney']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=syncmoney&m=weixuexiao');

        $result['data']['onlineflow']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=onlineflow&m=weixuexiao');
        $result['data']['checkforks']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=checkforks&m=weixuexiao');
        $result['data']['cardinfo']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=cardinfo&m=weixuexiao');
        $result['data']['banpai']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=banpai&m=weixuexiao');
        $result['data']['getallroomurl']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=getallroom&m=weixuexiao');
        $result['data']['all']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=all&m=weixuexiao');
        $result['data']['allcard']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=allcard&m=weixuexiao');
		$result['data']['busgps']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkxz&op=busgps&m=weixuexiao');
            ...
            if($AddMode > 0 ){ //如果是充值
                ...
                if(!empty($studentyue)){
                    ...
                    if($payKind == 5 ){ //直接操作补助
                        ...
                        pdo_update(GetTableName('students',false),array('buzhu'=>$afterpay),array('id'=>id));
                        //消费记录
                        $yuelog = array(
                            'schoolid'   => $schoolid,
                            'weid'       => $weid,
