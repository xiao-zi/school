 ...
 */
	...
	$operation = in_array ( $_GPC ['op'], array ('default', 'login', 'classinfo', 'check', 'gps', 'banner', 'video','getleave','getroomlist','onlineflow','offlineflow','busgps') ) ? $_GPC ['op'] : 'default';
	$weid = $_GPC['i'];
...
$school['xk_type'] =$xk_type['xk_type'];
	...
	if ($operation == 'login') {
		if(!empty($ckmac)){
			...
			$result['data']['CHECK_URL'] = 	urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkhx&op=check&m=weixuexiao');
			$result['data']['LEAVE_URL'] = 	urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkhx&op=getleave&m=weixuexiao');
			$result['data']['OUTTIME_URL'] = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkhx&op=classinfo&m=weixuexiao');	
            $result['data']['getroomlisturl']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkhx&op=getroomlist&m=weixuexiao');
			$result['data']['busgps']   = urlencode($_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&schoolid=' . $schoolid . '&do=checkhx&op=busgps&m=weixuexiao');
			$result['code'] = 1000;
			$result['msg'] = "success";
