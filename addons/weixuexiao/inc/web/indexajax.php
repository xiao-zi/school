...
if ($operation == 'recreateqr')  {
	...
	if (! $_GPC ['id']) {
		...
		if (!is_error($result)) {
			...
			$pathname_old = "images/weixuexiao/".$arr_old['2'];
			$pathname = "images/weixuexiao/".$arr['2'];
			if (!empty($_W['setting']['remote']['type'])) { // 
...
if ($operation == 'buy_kc') { 
	...
	$sidarr = $_GPC['sidarr'];
	$tid = $_GPC['tid'];
	$kcinfo = pdo_fetch("SELECT id,cose,FirstNum,payweid,overtimeday FROM ". tablename($this->table_tcourse)." WHERE id = :id And schoolid = :schoolid", array(':id' => $kcid,':schoolid' => $schoolid));
	$falseStu = '';
	$count = 0;
	foreach($sidarr as $value)
	{
		...
		if(!empty($check)){
			...
			if(pdo_insert($this->table_order,$tempOrder)){
				...
				if($couseid){
					pdo_update($this->table_coursebuy,array('orderid'=>$orderid),array('id' => $couseid));
				}
			}else{
				$falseStu .= $student['s_name']."/////"; 
			}
...
if ($operation == 'checkver') {
	$checkbb = pdo_fetch("SELECT mid FROM ".tablename('modules')." WHERE :name = name ", array(':name' => 'weixuexiao'));
	...
	$result ['result'] = true;
	$result ['url'] = '/index.php?c=module&a=manage-system&do=module_detail&&name=weixuexiao&support=&type=1';
	die ( json_encode ( $result ) );
}
