 ...
 */
	...
	if ($operation == 'display1') {
		...
		foreach($wechats as $key =>$row){
			$wechats[$key]['cert'] = IA_ROOT.'/addons/weixuexiao/public/cert/'.$row['uniacid'].'/apiclient_cert.pem';
			$wechats[$key]['key'] = IA_ROOT.'/addons/weixuexiao/public/cert/'.$row['uniacid'].'/apiclient_key.pem';
		...
		foreach($wxapps as $key =>$row){
			$wxapps[$key]['cert'] = IA_ROOT.'/addons/weixuexiao/public/cert/'.$row['uniacid'].'/apiclient_cert.pem';
			$wxapps[$key]['key'] = IA_ROOT.'/addons/weixuexiao/public/cert/'.$row['uniacid'].'/apiclient_key.pem';
		...
		if(checksubmit('submit')){
			...
			if($_FILES['cert'] && $_FILES['key']){
				...
				if (!is_dir(IA_ROOT."/addons/weixuexiao/public/cert/". $acid."/")) {
					mkdirs(IA_ROOT."/addons/weixuexiao/public/cert/". $acid."/", "0777");
				}
				...
				file_put_contents(IA_ROOT. "/addons/weixuexiao/public/cert/".$acid."/". $_FILES['cert']['name'], $cert);
				$key = file_get_contents($_FILES['key']['tmp_name']);
				$key_name = 'apiclient_key.pem';
				if ($key_name != $_FILES['key']['name']) {
					itoast('上传文件不合法,请重新上传', '', 'error');
				}
				file_put_contents(IA_ROOT. "/addons/weixuexiao/public/cert/".$acid."/". $_FILES['key']['name'], $key);
				message('上传成功');
			}else{
				itoast('请选择文件', '', 'error');
			}
