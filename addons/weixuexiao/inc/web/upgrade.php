...
define('weixuexiao_AUTH_URL', 'http://auth.7mao.cc/api/');
...
$versionfile = IA_ROOT . '/addons/weixuexiao/inc/version.php';
require $versionfile;
if ($op == 'display') {
	$updatedate = date('Y-m-d H:i', filemtime($versionfile));
    $version = weixuexiao_VERSION;
} else if ($op == 'sys') {

} else if ($op == 'check') {
    set_time_limit(0);
    $version = defined('weixuexiao_VERSION') ? weixuexiao_VERSION : '1.0';
    $resp = ihttp_post(weixuexiao_AUTH_URL, array(
        'type' => 'check',
        'ip' => $_W['clientip'],
        'module' => $_GPC['m'],
    ...
    if (!empty($path)) {
        ...
        if (!empty($upgrade['upgrade'])) {
            $updatefile = IA_ROOT . "/addons/weixuexiao/upgrade.php";
            file_put_contents($updatefile, base64_decode($upgrade['upgrade']));
            require $updatefile;
            @unlink($updatefile);
        }
        $ifile = IA_ROOT . "/addons/weixuexiao/install.php";
        $upfile = IA_ROOT . "/addons/weixuexiao/upgrade.php";
        if (is_file($ifile)) {@unlink($ifile);}
        if (is_file($upfile)) {@unlink($upfile);}
        load()->func('file');
        @rmdirs(IA_ROOT . "/addons/weixuexiao/tmp");
        file_put_contents(IA_ROOT . "/addons/weixuexiao/inc/version.php", "<?php if(!defined('IN_IA')) {exit('Access Denied');}if(!defined('weixuexiao_VERSION')) {define('weixuexiao_VERSION', '" . $upgrade['version'] . "');}if(!defined('weixuexiao_TIME')) {define('weixuexiao_TIME', '" . $upgrade['uptime'] . "');}");
    ...
    file_put_contents(IA_ROOT . "/addons/weixuexiao/inc/version.php", "<?php if(!defined('IN_IA')) {exit('Access Denied');}if(!defined('weixuexiao_VERSION')) {define('weixuexiao_VERSION', '1.0');}");
