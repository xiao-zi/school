			...
			if(checksubmit('submit')){
				...
				if(empty($id)){
                    ...
                    /* if(keep_wt() && CheckWtOn($schoolid)) {
                        mload()->model('wt');
                        //人员录入
                        $param['idcardNo'] = $thistid;
                        $param['name']     = $data['tname'];
                        $result            = personAction($schoolid, $weid, time(), $param, 'insert','tea');
                        if($result['result'] == '1'){
                            $guid =$result['data']['guid'];
                            pdo_update(GetTableName('teachers',false),array('guid'=>$guid),array('id'=>$thistid));
                            $result_device =  People2Device($schoolid, $weid, time(),$guid);
                            if($result_device['result'] == '1') {
                                $imgurl      = tomedia($data['thumb']);
                                $result_face = PersonFace($schoolid, $weid, time(), $guid, $imgurl);
                                if ($result_face['result'] == '1') {
                                    pdo_update(GetTableName('teachers', false), array('photo_guid' => $result_face['data']['guid']), array('id' => $thistid));
                                }else{
                                    $back_msg = CheckWtReturnCode($result_face['code']);
                                    $this->imessage($back_msg);
                   ...
                   /*  if(keep_wt() && CheckWtOn($schoolid)) {
                        mload()->model('wt');
                        //人员更新
                        $guid = pdo_fetch("SELECT guid,photo_guid FROM " . GetTableName('teachers') . " WHERE id = :id", array(':id' => $id));
                        ...
                        if($result['result'] == '1') {
                            ...
                            if ($result_face['result'] == '1') {
                                pdo_update(GetTableName('teachers', false), array('photo_guid' => $result_face['data']['guid']), array('id' => $id));
                            }else{
                                $back_msg = CheckWtReturnCode($result_face['code']);
                            }
				...
				$this->imessage($urlMsg, $this->createWebUrl('assess', array('op' => 'display', 'schoolid' => $schoolid)), 'success');
			}
