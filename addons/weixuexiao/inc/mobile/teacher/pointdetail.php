 ...
 */
		...
		if($thistime){
		    ...
		    $list1 = pdo_fetchall("SELECT * FROM " . tablename($this->table_pointsrecord) . " where  schoolid=:schoolid AND tid = :tid $condition  ORDER BY createtime DESC LIMIT 0,10" , array( ':schoolid' => $schoolid, ':tid'=> $tid));
				
			foreach($list1 as $key => $row){
				$point = pdo_fetch("SELECT * FROM " . tablename($this->table_points) . " where schoolid=:schoolid AND id = :id", array( ':schoolid' => $schoolid, ':id'=> $row['pid']));
		    ...
		    $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_pointsrecord) . " where  schoolid=:schoolid AND tid = :tid   ORDER BY createtime DESC LIMIT 0,10" , array(':schoolid' => $schoolid, ':tid'=> $tid));
