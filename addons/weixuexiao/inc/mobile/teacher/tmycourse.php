		...
		if($it){
            ...
            if($notOwner =='notOwner'){
				...
				if($nj_id != 0){
					...
					$kcstr = trim($kcstr,',');
					$allsign =  pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_kcsign) . " WHERE schoolid = '{$schoolid}' And tid = {$it['tid']} and sid = 0 And status= 2 and FIND_IN_SET(kcid,'{$kcstr}') ");
					$gudingsign = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_kcsign) . " WHERE schoolid = '{$schoolid}' And tid = {$it['tid']} and sid = 0 And status= 2 And type=0 and  FIND_IN_SET(kcid,'{$kcstr}')");
					$freesign = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_kcsign) . " WHERE schoolid = '{$schoolid}' And tid = {$it['tid']} and sid = 0 And status= 2 And type =1 and FIND_IN_SET(kcid,'{$kcstr}')" );
					...
					$allsign =  pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_kcsign) . " WHERE schoolid = '{$schoolid}' And tid = {$it['tid']} and sid = 0 And status= 2 ");
					$gudingsign = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_kcsign) . " WHERE schoolid = '{$schoolid}' And tid = {$it['tid']} and sid = 0 And status= 2 And type=0 ");
					$freesign = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_kcsign) . " WHERE schoolid = '{$schoolid}' And tid = {$it['tid']} and sid = 0 And status= 2 And type =1");
				}
