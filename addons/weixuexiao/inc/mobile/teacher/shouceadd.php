...
//1
        ...
        if(!empty($userid['id'])){
			...
			$allkc = pdo_fetchall("SELECT id,name FROM " . tablename($this->table_tcourse) . " where schoolid = '{$schoolid}' And weid = '{$weid}' And (tid like '{$it['tid']},%' OR tid like '%,{$it['tid']}' OR tid like '%,{$it['tid']},%' OR tid='{$it['tid']}') ORDER BY id ASC, ssort DESC");
