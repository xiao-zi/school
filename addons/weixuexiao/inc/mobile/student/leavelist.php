...
//1
		...
		if(!empty($it)){
			...
			foreach($xsqj as $key => $row){
				$user = pdo_fetch("SELECT * FROM " . tablename($this->table_user) . " where (uid = :uid And openid = :openid or userid = :userid) And sid = :sid ", array(':uid' => $row['uid'],':openid' => $row['openid'],':sid' => $row['sid'],':userid'=>$row['user_id']));
				if($user['pard'] ==2){
					$xsqj[$key]['guanxi'] = "妈妈";
				}
				if($user['pard'] ==3){
