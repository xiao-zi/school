...
//1
		...
		if($students['status'] == 1){//未解锁的学生进入个人中心
			$stopurl = $_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&do=useredit&m=weixuexiao'.'&schoolid='.$schoolid.'&id='.$_SESSION['user'];;
			header("location:$stopurl");
			exit;
		}else{
			...
			$school = pdo_fetch("SELECT title,spic,is_rest,shoucename,is_video,videoname,is_zjh,is_recordmac,style2,userstyle,gonggao,copyright,Is_point,is_chongzhi,is_shoufei,is_buzhu FROM " . tablename($this->table_index) . " where weid = :weid AND id=:id ", array(':weid' => $weid, ':id' => $schoolid));
			$guid = need_guid($it['id'],$schoolid,1);
			if(!empty($guid)){
				pdo_update($this->table_user, array('is_frist' => 2), array('id' => $it['id']));	
				$stopurl = $_W['siteroot'] . 'app/index.php?i=' . $weid . '&c=entry&do=guid&m=weixuexiao'.'&schoolid='.$schoolid.'&guid='.$guid.'&place=user';;
				header("location:$stopurl");
				exit;
			}
		}	
