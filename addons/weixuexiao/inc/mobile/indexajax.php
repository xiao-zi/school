		...
		if($_GPC['opt'] == 'check'){
			...
			foreach($ksarr as $row){
				...
				if($kcinfo && $ksinfo){
					...
					pdo_insert($this->table_leave, $data);
					$leave_id = pdo_insertid();
					$this->sendMobileXsqj($leave_id, $schoolid, $weid, $ksinfo['maintid']);
				}
			}
			$data ['result'] = true;
			$data ['msg'] = '已将请假申请发送至覆盖课程的主讲老师，请勿重复申请！';
		}
	}
...
if ($operation == 'changePimg') {
    ...
    if (empty($user['id'])) {
        ...
        if(!empty($photoUrl)) {
            ...
            $url = 'https://file.api.weixin.qq.com/cgi-bin/media/get?access_token='.$token2.'&media_id='.$photoUrl;
            $pic_data = ihttp_request($url);
            $path = "images/weixuexiao/cardthumb/".$user['schoolid']."/";
