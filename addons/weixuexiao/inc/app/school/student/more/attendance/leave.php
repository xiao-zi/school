...
//班级信息
$class = pdo_fetch("SELECT tid FROM " . tablename($this->table_classify) . " WHERE sid = '{$student['bj_id']}' AND type = 'theclass' ");
//班主任信息
$teacher = pdo_fetch("SELECT tname FROM " . tablename($this->table_teachers) . " WHERE id = '{$class['tid']}' ");
if(empty($teacher)){
    json_encodeBack(array('status'=>10003,'msg'=>'该学生的班级尚未安排班主任职位,请联系校方！'));
}
$result = array(
    ...
    'teacher_id'=>$class['tid'],//班主任id
    'teacher_name'=>$teacher['tname'],//班主任名字
    'student_name'=>$student['s_name'],//学生名字
);
