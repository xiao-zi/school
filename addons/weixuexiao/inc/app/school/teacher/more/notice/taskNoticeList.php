...
if(!$teacher_model->getRole($teacher_id,2000301,$school_id,2)){
...
$leave = pdo_fetchall("SELECT id,title,tid,createtime,content,ismobile,usertype FROM " . tablename($this->table_notice) . " where  schoolid = '{$school_id}' And type = 3 $condition ORDER BY createtime DESC LIMIT $limitStr");
