...
//通知信息
$notice = pdo_fetch("SELECT id,title,tname,createtime,type,tid,content,video,videopic,audio,audiotime,picarr,comment,is_research,bj_id,usertype,userdatas FROM " . tablename($this->table_notice) . " where id = '{$id}' And type = 1");
if(empty($notice)){
    json_encodeBack(array('status'=>10003,'msg'=>'非法请求！'));
}
//检查该老师是否是该班级的年级主任
$checkGradeDirector = $teacher_model->checkGradeDirector($teacher_id,$notice['bj_id']);
//查看结果
$viewResult = false;
if (($teacher['status'] == 2 || $notice['tid'] == $teacher_id || $checkGradeDirector) || ($notice['is_research'] != 1 || $teacher_model->getRole($teacher_id,2000104,$school_id,2))){
    $viewResult = true;
}
//图片处理
