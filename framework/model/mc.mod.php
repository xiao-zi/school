...
function mc_oauth_account_userinfo($url = '') {
    ...
    $_SESSION['openid'] = 'dwqwqqqqqjvdsjweljfqwp2';
...
//		$userinfo = $oauth_account->fansQueryInfo($_SESSION['openid']);
            $userinfo = array(
                'openid'=>'dwqwqqqqqjvdsjweljfqwp2',
                'unionid'=>'2dwqjdwqjwdwq56d29832fj',
                'nickname'=>'贺杭伟',
                'headimgurl'=>'http://school.test/attachment/images/1/2020/04/c4cYnHyyZ8VSj83YHryt58ypRCYQTe.jpg',
                'sex'=>1,
                'city'=>'西安',
                'province'=>'陕西',
                'country'=>'中国'
		...
		if (!is_error($userinfo) && !empty($userinfo) && is_array($userinfo) && !empty($userinfo['nickname'])) {
			...
			if (!empty($fan['uid']) || !empty($_SESSION['uid'])) {
                ...
                $mc_members = pdo_fetch("SELECT uid FROM " . tablename ( 'mc_members' ) . " where uniacid = '{$_W['uniacid']}' And uid = '{$uid}' ");
                if (!empty($record)) {
