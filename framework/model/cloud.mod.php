...
define('CLOUD_GATEWAY_URL', HTTP_X_FOR .'127.0.0.1/app/api.php');
define('CLOUD_GATEWAY_URL_NORMAL', HTTP_X_FOR .'127.0.0.1/app/api.php');
...
function cloud_prepare() {
    ...
    if (empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
        return error('-1', "您的站点只有在微擎云服务平台成功注册后，才能使用云服务的相应功能，<br />请联系QQ：396049809，不用购买授权不影响使用请忽略");
    }
    return true;
}
function cloud_build() {
    ...
    $dat = cloud_request(HTTP_X_FOR .'127.0.0.1/app/api.php', $pars);
...
function cloud_download($path, $type = '') {
    ...
    $dat = cloud_request(HTTP_X_FOR .'127.0.0.1/app/api.php', $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_sms_info() {
    ...
    $res=file_get_contents("http://api.smsbao.com/balance?u={$sms_name}&p={$sms_password}");
    $retArr = explode(",", $res);
    $balanceArr = $retArr[1];
    $data['sms_count']=$retArr[0] == 0 ? $balanceArr:0;
    return $data;
}
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request(HTTP_X_FOR .'127.0.0.1/app/api.php', $pars);
...
function cloud_auth_url($forward, $data = array()) {
    ...
    $auth_url = HTTP_X_FOR .'127.0.0.1/api/auth.php?__auth=' . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request(HTTP_X_FOR .'127.0.0.1/app/api.php', $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
