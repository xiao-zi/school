...
var kccommend = <?php  echo $lastid['id'];?>;
var kccommends = kccommend + 22;
var kcteampic = 0;
	$('#kccommend').click(function(){
		kcteampic++;
		kccommends++;
		
		var imgurlss = "url"+kccommends;
		var imgurls = "'"+imgurlss+"'";
		var htmls1ss = '<div id="iconeditor'+kccommends+'" class="editor needhidden" style="top: 10px;">'+
					'	<div class="ng-scope">'+
					...
					'						<input type="hidden" name="type['+kccommends+']" value="2" />'+
					'						<input type="hidden" name="place['+kccommends+']" value="22" />'+
					'						<div class="form-group">'+
					'							<label class="col-xs-3 control-label"><span class="red">*</span>课程名称['+kcteampic+']</label>'+
					'							<div class="col-xs-9">'+
					'								<input type="text" id="btnname'+kccommends+'" name="btnname['+kccommends+']" onkeyup="SwapTxt1('+kccommends+')" placeholder="请输入课程名称" value=" " class="form-control ng-pristine ng-untouched ng-valid input_this">'+
					'							</div>'+
					'						</div>'+										
					...
					'										<input type="text" value="" id = "url'+kccommends+'" name="url['+kccommends+']" class="form-control" autocomplete="off">'+
					...
					'							<div class="col-xs-9">'+
					'								<div class="input-group ">'+
					'									<input type="text" name="iconpics['+kccommends+']" id="iconpics'+kccommends+'" class="form-control" autocomplete="off" filename="" url="">'+
					'									<span class="input-group-btn">'+
					'										<button class="btn btn-default" type="button" data_id="mofang'+kccommends+'" onclick="showImageDialogmfs(this);">选择图片</button>'+
					'									</span>'+
					'								</div>'+
					'								<div class="input-group " style="margin-top:.5em;">'+
					'									<img id="imgsrc_this'+kccommends+'" src="./resource/images/nopic.jpg" onerror="" class="img-responsive img-thumbnail" width="150">'+
