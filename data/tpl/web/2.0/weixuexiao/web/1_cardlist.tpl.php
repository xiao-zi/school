			...
			<form action="./index.php" method="get" class="form-horizontal" role="form">
				...
				<div class="form-group">
					...
					<div class="col-sm-2 col-lg-2">
						...
						<a class="btn btn-primary">启用中 <i class="fa fa-spinner fa-spin"></i></a>
						<?php  } else { ?>
				...
				<div class="form-group">
					...
					<div class="col-sm-2 col-lg-2">
						<a class="btn btn-success qx_2503" href="javascript:;" onclick=" $('#Modal2').modal('toggle');">班级绑卡</a>
					</div>

					<?php  if(keep_wt() && CheckWtOn($schoolid)) { ?>
					<div class="col-sm-2 col-lg-2">
						<a class="btn btn-success qx_2503" href="<?php  echo $this->createWebUrl('cardlist', array('op' => 'OneKeyUpCard', 'schoolid' => $schoolid))?>" >一键同步卡信息</a>
					</div>
	...
	<div class="panel panel-default file-container" style="display:none;">
		<div class="panel-body">
			<form id="form">
				...
				<a class="btn btn-info" href="../addons/weixuexiao/public/example/example_cardlist.xls"><i class="fa fa-download"></i>下载导入模板</a>
			</form>
		</div>
	</div>
	<div class="panel panel-default file-container1" style="display:none;">
		<div class="panel-body">
			<form id="form1">
				...
				<a class="btn btn-primary" href="../addons/weixuexiao/public/example/example_cardlist1.xls">下载导入模板</a>
			...
			<form action="" method="post" class="form-horizontal form" enctype="multipart/form-data">
				...
				<table class="table table-hover">
					...
					<tr>
						...
						<td>
							<?php  if(!empty($item['tid']) || !empty($item['sid'])) { ?>
							<img style="width:35px;height:35px;border-radius:50%;" src="<?php  if($item['scardicon']) { ?><?php  echo $item['scardicon'];?><?php  } ?><?php  if($item['tcardicon']) { ?><?php  echo $item['tcardicon'];?><?php  } ?>" onerror="this.src='./resource/images/nopic.jpg';" width="50"  style="border-radius: 3px;" /></br><?php  echo $item['pname'];?>
							<?php  } ?>
							
							<?php  if($item['cardtype'] == 2) { ?>
							<span class="label label-info">班级卡</span>
							<?php  } ?>
						</td>
						<td><?php  if($item['s_name']) { ?><img style="width:35px;height:35px;border-radius:50%;" src="<?php  if($item['sid']) { ?><?php  echo $item['sicon'];?><?php  } ?>" width="50"  style="border-radius: 3px;" /></br><?php  echo $item['s_name'];?><?php  } ?> <?php  if($item['cardtype'] == 2) { ?>
							<span class="label label-info">班级卡</span>
							<?php  } ?></td>
						<td><?php  if($item['tname']) { ?><img style="width:35px;height:35px;border-radius:50%;" src="<?php  if($item['tid']) { ?><?php  echo $item['ticon'];?><?php  } ?>" width="50"  style="border-radius: 3px;" /></br><?php  echo $item['tname'];?><?php  } ?> <?php  if($item['cardtype'] == 2) { ?>
							<span class="label label-info">班级卡</span>
							<?php  } ?></td>
						<td>
						...
						<td><?php  if($item['sid']) { ?><?php  echo $item['bjname'];?><?php  } ?></td>
						<td>
							<?php  if((empty($item['sid']) && empty($item['tid']) && $item['cardtype'] == 1) || ($item['cardtype'] == 2 && empty($item['bj_id']))) { ?><span class="label label-danger">未绑定</span><?php  } else { ?><?php  if(!empty($item['num'])) { ?><span class="label label-success"><?php  echo $item['num'];?>次</span><?php  } else { ?><span class="label label-success">0次</span><?php  } ?><?php  } ?>
						</td>
						<td>
						...
						<td class="qx_2504">
							<?php  if((empty($item['sid']) && empty($item['tid']) && $item['cardtype'] == 1) || ($item['cardtype'] == 2 && empty($item['bj_id']))) { ?><span class="label label-danger">未绑定</span><?php  } else { ?>
							<?php  if($item['cardtype'] != 2) { ?>
							<a class="btn btn-default btn-sm" href="<?php  echo $this->createWebUrl('cardlist', array('id' => $item['id'], 'op' => 'jiebang', 'schoolid' => $schoolid))?>" onclick="return confirm('此操作不可恢复，确认解绑？');return false;" title="解绑"><i class="fa fa-times"></i>&nbsp;解绑</a><?php  } else { ?><span class="label label-default">班级卡禁止解绑</span><?php  } ?>
							<?php  } ?>
						</td>
						<td class="qx_e_d" style="text-align:right;">
							<?php  if($item['cardtype'] != 2) { ?>
							<a class="btn btn-default btn-sm qx_2502" href="<?php  echo $this->createWebUrl('cardlist', array('id' => $item['id'], 'op' => 'post', 'schoolid' => $schoolid))?>" title="编辑"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;<?php  } ?><a class="btn btn-default btn-sm qx_2505" href="<?php  echo $this->createWebUrl('cardlist', array('id' => $item['id'], 'op' => 'delete', 'schoolid' => $schoolid))?>" onclick="return confirm('此操作不可恢复，确认删除？');return false;" title="删除"><i class="fa fa-times"></i></a>
						</td>
					</tr>
					<?php  } } ?>
					</tbody>
					<tr>
						<td colspan="10">
							...
							<?php  if(keep_wt() && CheckWtOn($schoolid)) { ?>
							<input name="token" type="hidden" value="<?php  echo $_W['token'];?>" />
							<input type="button" class="btn btn-primary qx_2505" name="ForceSync" value="批量强制同步">
							<?php  } ?>
						</td>
					</tr>
				</table>
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header" style="color: black;">
				<h4 class="modal-title" id="ModalTitle1">绑定班级卡</h4>
			</div>
				...
				<form id="upsence_form" method="post" class="form-horizontal form">
					...
					<div class="panel panel-default">
						<div class="panel-body">
							...
							<div class="form-group">
								<label class="col-xs-12 col-sm-2 col-md-2 col-lg-2 control-label" style="width: 100px;">录入卡</label>
								<div class="col-sm-2 col-lg-3">
									<input class="form-control" name="keyword" id="cardId" type="text" value="">
								</div>

								<label class="col-xs-12 col-sm-2 col-md-2 col-lg-2 control-label" style="width: 100px;">到期时间</label>
								<div class="col-sm-2 col-lg-3">
									<?php  echo tpl_form_field_date('serverend', date('Y-m-d'))?>
								</div>
							</div>
						</div>
					</div>
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-primary" id="submit1" onclick="bjcardBD()">确定绑定</button>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
			...
			<?php  } ?>
				...
				<?php  } ?>
					...
					<?php  } ?>
						...
						$("input[name=ForceSync]").click(function () {
							var check = $("input[type=checkbox][class!=check_all]:checked");
							if(check.length < 1){
								alert('请选择要强制同步的卡号!');
								return false;
							}
							if(confirm("确认要强制同步选中卡号，覆盖之前的人员信息?")){
								var id = new Array();
								check.each(function(i){
									id[i] = $(this).val();
								});
								var url = "<?php  echo $this->createWebUrl('cardlist', array('op' => 'FOrceSync','schoolid' => $schoolid))?>";
								$.post(
										url,
										{idArr:id},
										function(data){
	...
	function bjcardBD(){
		...
		let serverendtime = $("input[name=serverend]").val();
		$.ajax({
			url: "<?php  echo $this->createWebUrl('cardlist',array('op'=>'bjcardbd','schoolid'=>$schoolid))?>",
			type: "POST",
			dataType: "json",
			data:{
				bj_id : BJid,
...
<div class="main">
	<form action="" method="post" class="form-horizontal form" enctype="multipart/form-data">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<div class="alert alert-info">
					注意：修改卡号，原卡会作废，如要继续使用原卡，需要重新联系管理员启用该卡
					<?php  if($_W['isfounder'] || $_W['role'] == 'owner') { ?></br>如因修改卡号导致原来的卡作废，废卡可自行导入卡库再次使用-------此句仅限管理员查看<?php  } ?>
				</div>
				<?php  if(!empty($item['sid']) || !empty($item['tid'])) { ?>
				<?php  if(!empty($item['sid'])) { ?>
				<div class="form-group">
					<label class="col-sm-3 col-md-2 control-label">学生姓名:</label>
					<label class="radio-inline" style="color:red;">
						<?php  echo $student['s_name'];?>
					</label>
				</div>
				<div class="form-group">
					<label class="col-sm-3 col-md-2 control-label">持卡人姓名:</label>
					<div class="col-sm-2">
						<input type="text" name="pname" value="<?php  echo $CardPname;?>" class="form-control" />
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-3 col-md-2 control-label">持卡人头像:</label>
					<div class="col-sm-2">
						<?php  echo tpl_form_field_image('spic', $item['spic'])?>
						<div class="help-block">注意，修改头像只支持全新上传</div>
					</div>
				</div>
				<?php  } ?>
				<?php  if(!empty($item['tid'])) { ?>
				<div class="form-group">
					<label class="col-sm-3 col-md-2 control-label">教师姓名:</label>
				...
				<div class="form-group">
					<label class="col-sm-3 col-md-2 control-label">持卡人头像:</label>
					<div class="col-sm-2">
						<?php  echo tpl_form_field_image('tpic', $item['tpic'])?>
						<div class="help-block">注意，修改头像只支持全新上传,修改教师卡头像将会同时修改教师头像</div>
					</div>
				</div>
				<?php  } ?>
				<?php  } ?>
				<?php  if(!empty($item['bj_id']) && !empty($item['sid'])) { ?>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<label class="radio-inline">
							<input type="radio" name="card_from" value="1" id="credit1">卡库
						</label>
						<label class="radio-inline">
							<input type="radio" name="card_from" value="2" id="credit2">手动
						</label>
					...
					<div class="form-group">
						...
						<div class="col-sm-2 col-lg-2">
							<select class="form-control" name="idcard_kk">
								...
								<option value="<?php  echo $row['id'];?>" ><?php  echo $row['idcard'];?></option>
								<?php  } } ?>
							</select>
		...
		<div class="form-group col-sm-12">
			...
			<input class="btn btn-primary col-lg-1" value="返回" onclick="javascript :history.back(-1);" />
		</div>
	</form>
</div>
<script type="text/javascript">
