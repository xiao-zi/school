...
<body class="auto_height" id="auto_height" style="">
		...
		<section>
		    <form id="frmAddress" action="<?php  echo $this->createMobileUrl('goodsdetail',array('schoolid' => $schoolid,'id' => $userid['id']), true)?>" method="POST">
		        ...
		        <div class="name">
		            ...
		            <input type="text" name="name" id="username" value="<?php  echo $MyAddress['name'];?>" placeholder="请填写你的名字">
		        </div>
		        
		        <!--收货电话号码-->
		        <div class="phone_number">
		            <label class="titleDescribe" for="phone_number" style="width:70px !important">联系电话:</label>
		            <input type="text" maxlength="11" oninput="value=value.replace(/[^\d]/g,'')" name="phone" placeholder="请填写你的联系方式" id="phone_number" value="<?php  echo $MyAddress['phone'];?>">
		        </div>

		        <!--收货地址-->
		        <div class="address">
		            <label class="titleDescribe" for="phone_number" style="width:70px !important">所在地区:</label>
		            <select id="province" class="stu_info_text" data-index="<?php  echo $MyAddress['province'];?>"><option value="">省份</option></select>
		            <select id="city" class="stu_info_text" data-index="<?php  echo $MyAddress['city'];?>"><option value="">城市</option></select>
		            <select id="county" class="stu_info_text" data-index="<?php  echo $MyAddress['county'];?>"><option value="">地区</option></select>
		        ...
		        <div class="addressDetails">
	           		<label class="titleDescribe" for="phone_number" style="width:80px !important">详细地址:</label> <input type="text" name="address" id="address" value="<?php  echo $MyAddress['address'];?> " placeholder="请填写你的收货地址">
		        </div>
		    </form>
	...
	<script src="<?php echo OSSURL;?>public/mobile/js/common.js?v=0622"></script><div id="common_progress" class="common_progress_bg">
...
<script>
    ...
    function gobackhome_fun() {
     ...
     <script src="<?php echo OSSURL;?>public/mobile/js/region_data.js?v=0622"></script>
    <script src="<?php echo OSSURL;?>public/mobile/js/region_inc.js?v=0622"></script>
    <script>
        function sumbit() {
            ...
            $.post("<?php  echo $this->createMobileUrl('indexajax',array('op'=>'xgdz'))?>",{"addressid":<?php  echo $addressid;?>, "name":AddName,"phone":AddPhone, "schoolid":<?php  echo $schoolid;?>, "province":province,"city":city,"county":country,"address":detailAdd},function(data){
					if(data.result){
						jTips(data.info);
						window.history.back();
					}else{
		...
		<?php  } else if(empty($addressid)) { ?>
		  $.post("<?php  echo $this->createMobileUrl('indexajax',array('op'=>'xgdz'))?>",{"name":AddName,"phone":AddPhone, "schoolid":<?php  echo $schoolid;?>, "province":province,"city":city,"county":country,"address":detailAdd},function(data){
					if(data.result){
						jTips(data.info);
						window.history.back();
					}else{
