...
<body style="">
	<div id="titlebar" class="header mainColor">
		...
		<div class="m">
			<span style="font-size: 18px">商品详情</span>   
		</div>
		<div class="r"></div>
	</div>
    ...
    <div>
        ...
        <button class="sure_change">我要兑换</button>
    </div>
</div>
<script>
setTimeout(function() {
    ...
    $(function () {
        $(".sure_change").click(function () {
           ...
           // if (address == "") { jTips("请输入地址"); $(".goods_send_address").click(); return false; }
        	var have = <?php  echo $Teacher['point'];?>;
           	var cost = <?php  echo $good['points'];?>;
           	if( have < cost )
           	{
	           	location.href = "<?php  echo $this->createMobileUrl('nopoint',array('schoolid' => $schoolid,'id' => $userid['id']), true)?>";
           	}else{
            location.href = "<?php  echo $this->createMobileUrl('editorder',array('schoolid' => $schoolid,'id' => $userid['id'],'goodsid' => $goodsid), true)?>";
        };
        });
    });
</script>
