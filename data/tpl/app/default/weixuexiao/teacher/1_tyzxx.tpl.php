...
<div class="user_info" id="huifu" style="display:none;">
	<div style="border-radius: 5%;">
		<ul>
			<input type="hidden" id="id" value="0">
			<p></p>
			<li class="user_name">
				<textarea rows="7" class="textareacss" placeholder="输入回复内容" id="huifuword" name="huifuword" maxlength="500"></textarea>
			</li>
		</ul>
...
<script>
	...
	$("#tijiao1").on('click', function () {
		...
		var huifuword = $("#huifuword").val();
			...
			$.post("<?php  echo $this->createMobileUrl('comajax',array('op'=>'huifu_mail'))?>",data,function(data){
				if(data.result){
					jTips(data.msg);
					location.reload();
				}else{
	...
	$(function() {
		...
		<?php  if(!IsHasQx($tid_global,2001802,2,$schoolid) ) { ?>
			$(".qx_01802").hide();
		<?php  } ?>
		new Scroll_load({
			"limit": "0",
			...
			"ajax_url": "<?php  echo $this->createMobileUrl('tyzxx', array('schoolid' => $schoolid,'op'=>'scroll'), true)?>",
