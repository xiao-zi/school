...
<div id="titlebar" class="header mainColor">
	...
	<div class="m"><a><span style="font-size: 18px">【<?php  echo $tea_show['tname'];?>】老师授课情况</span></a></div>
...
<div class="All">       
	...
	<div class="listContent">
		<?php  if(is_array($kclist)) { foreach($kclist as $key => $v) { ?>
			...
			<div class="notifyTopBox" style="height:auto;">
				...
				<div class="notifyTopRight" style="width:94%;padding-right: 10px;">
					...
					<div class="notifyTopRightTopBox" style="height:25px;margin-top: 2px;">
						...
						<span class="teacherInfo">授课地址：<?php  echo pdo_fetch("SELECT sname FROM " . tablename($this->table_classify) . " WHERE schoolid = :schoolid And sid = :id", array(':schoolid' => $schoolid,':id' => $v['adrr']))['sname'];?></span>
...
setTimeout(function() {
	if(window.__wxjs_environment === 'miniprogram'){
		$("#titlebar").hide();
		$("#titlebar_bg").hide();
		document.title="【<?php  echo $tea_show['tname'];?>】老师授课情况";
	}
}, 100);
...
// 底部加载更多
	new Scroll_load({
		...
		"ajax_url": "<?php  echo $this->createMobileUrl('tkcsigndetail', array('schoolid' => $schoolid,'id'=>$checktid), true)?>",
	   ...
	   strConvertHtml();
		...
		$('body').on('click',  '.main', function(e) {
			if (!$(e.target).parent().is('.notifyImgItem')) {
				...
				window.location.href = "<?php  echo $this->createMobileUrl('tmykcinfo', array('schoolid' => $schoolid,'notOwner'=>'notOwner','checktid'=>$checktid))?>"+ '&id=' + notifyUid;
			} 
		});
