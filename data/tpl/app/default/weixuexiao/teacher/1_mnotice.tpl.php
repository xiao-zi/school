...
<input type="hidden"  id="hide_weid"  value="<?php  echo $weid;?>">
		...
		<h3 class="title"><?php  echo $leave['title'];?></h3>
			...
			<?php  if(( (!is_showpf() && ($teacher['status'] == 2 || $leave['tid'] == $it['tid'])) || (is_showpf() && ($leave['is_research'] != 1 || IsHasQx($tid_global,2000204,2,$schoolid)) ) )) { ?>
		...
		<div class="questionContent">
			<div class="questionBox">
				  <input type="hidden" id="txtQuestionnaireId" value="<?php  echo $leaveid;?>">
					  <?php  if(!empty($testAA)) { ?><span style="color:red;padding-left: 10px;"> 你已经提交过本次问卷，不能再提交了哦~</span></br></br>
...
<?php  echo $ZY_contents[$key]['content'][$keys]['title'];?><?php  if(((!empty($testAA))&&($ZY_contents[$key]['content'][$keys]['is_answer'] == "Yes"))) { ?><span style="color:red;">  【答案】</span><?php  } ?>
		...
		<?php  } ?>
		<?php  if($leave['usertype'] != 'school' || $leave['usertype'] != 'alltea' || $leave['usertype'] != 'allstu') { ?>
		<div class="commentBox">
			<div class="mobile_comment" >
				<div class="commentTitle">
					<span class="l"><?php  if($leave['usertype'] == 'send_class') { ?>指定班级<?php  } ?><?php  if($leave['usertype'] == 'student') { ?>指定学生<?php  } ?><?php  if($leave['usertype'] == 'staff_jsfz') { ?>指定老师组<?php  } ?><?php  if($leave['usertype'] == 'staff') { ?>指定老师<?php  } ?></span>
					
				</div>
				<ul class="commentList">
					<li class="comment_li">
					<?php  if(is_array($arr)) { foreach($arr as $row) { ?>
						<?php  if($leave['usertype'] == 'send_class' || $leave['usertype'] == 'staff_jsfz') { ?>
