...
<div class=" manual_list">
    ...
    <div class="manual_main_box">
        <div class="title_l">创建<?php  echo $school['shoucename'];?></div>
        <div class="blank"></div>
		...
		<div class="feedback_title_box">
			<select class="feedback_title feedback_title_teacher" id="qh_id">
				...
				<?php  if(is_array($allqh)) { foreach($allqh as $row) { ?>			
				<option value="<?php  echo $row['sid'];?>"><?php  echo $row['sname'];?></option>
				<?php  } } ?>				
			</select>
		</div>
        ...
        <div class="select_cover_img">
			<?php  if(is_array($allscset)) { foreach($allscset as $row) { ?>
                <div setid="<?php  echo $row['id'];?>" class="select_cover_li<?php  if($row['ssort'] ==1) { ?> on<?php  } ?>" style="background-image: url(<?php  echo tomedia($row['icon'])?>)"><p style="font-size:10px;"><?php  echo $row['title'];?></p></div>
			<?php  } } ?>
        </div>
        <div class="blank"></div>
		...
		$('.blue_add_btn').on('click', function () {
			...
			if (start_time == '') {
				...
				$.ajax({
					...
					data: { 'title': title,'tid': "<?php  echo $it['tid'];?>", 'setid': setid, 'bj_id': bj_id, 'qh_id': qh_id, 'kc_id': kc_id, 'starttime': start_time, 'endtime': end_time},
					dataType: 'json',
					type: 'post',
