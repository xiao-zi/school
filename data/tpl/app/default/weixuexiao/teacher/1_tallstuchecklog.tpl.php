...
<body id="kqtjbody">
		...
		<div class="component-dialog dialog-order reveal-modal" id="zixunkefu" style="z-index: 109;left:5vw;width:calc(90vw - 36px)" >
			<div class="component-dialog dialog-order"  id="detail_range" style="box-sizing: border-box;">
				...
				<div class="component-dialog-body">
                    ...
                    <div class="form-line">
						<div class="input-wrapper" style="border:none;">
							<span style="padding: 8px;float: left;color:gray">选择年级</span></br>
                            <div style="width:100%;padding-top:7px;position: relative;height: 40px" >
                    ...
                    <div class="form-line">
						<div class="input-wrapper" style="border:none;">
							<span style="padding: 8px;float: left;color:gray">选择班级</span></br>
							<div style="width:100%;padding-top:7px;position: relative;height: 40px" >
                                <span class="Select-event" style="position: absolute; top: 13px; left: 0px;width:45%" >{{BjName}}</span>
								<select @change="ChangeBj()" id="BjSelect" v-model="BjListKey" class="Select-event"  style="position: absolute; top: 13px; left: 0px;width:45%;opacity: 0;" >
										<?php  if($teacher['status'] == 2 || $teacher['status'] == 3) { ?>
	...
	<div id="main_detail"  style="margin-left: 3px;float:unset;margin-left: 1%;width:98%;background-color: white;border-radius: 8px 8px 0px 0px;overflow: hidden;">
		<div class="trhead" style="width:calc(100% - 1px);height: 40px;">
			...
			<div class="thhead" style="width:50%;">概述</div>
	...
	<div style="position: fixed;bottom: 70px;right: 10px;height: 50px;width: 50px;" onclick="backToTOP()">
		<img src="<?php echo MODULE_URL;?>public/mobile/img/backtotop_icon.png" style="height: 40px;width:40px;border-radius: 50%;">
	</div>		
    ...
    if (parseInt(winHeight) + parseInt(scrollTop) + 5 > parseInt(documentHeight)) {
        ...
        if (self.ajax_switch) {
            ...
            var post_data = { 
                ...
                Bjid    : TopVue.DoneData.BjListKey,
            };
            $('.has_show_over').show();
            $.ajax({
                type: 'POST',
...
var TopVue = new Vue({
	...
	data: function () {
		return{
			...
			NjName   : "<?php  echo $NjN;?>",
			BjName   : "<?php  echo $BjN;?>",
			NjListKey: <?php  echo $NjKey;?>,
			BjListKey: <?php  echo $BjId;?>,
			BjList	 :[],
	...
	methods:{
		ChangeNj:function(){
			if(this.NjListKey != -1){
				...
				this.DoneData.Njid = this.NjList[this.NjListKey]['sid']
			}else{
				this.BjList = [];
				this.NjName = "不限年级"	
				this.DoneData.Njid = -1
			}
		...
		ChangeDone:function(){
			...
			self.DoneData.InSch     = self.InSch
			self.DoneData.OutSch    = self.OutSch
			...
			self.DoneData.BjListKey = self.BjListKey
			self.DoneData.Bjid      = self.BjListKey
			self.DoneData.NjListKey = self.NjListKey
			
			$.ajax({
				url: "<?php  echo $this->createMobileUrl('tallstuchecklog', array('schoolid' => $schoolid,'op'=>'More_Data' ), true)?>",
				data : self.DoneData,	
				dataType: 'html',
				type: "post",
				success: function (data) {
...
function ChaKanXiangQing(id){
		...
		$.ajax({
			url: "<?php  echo $this->createMobileUrl('tallstuchecklog', array('schoolid' => $schoolid,'op'=>'GetDetail' ), true)?>",
			data : {
				id :id
			},	
			dataType: 'json',
...
new Scroll_load({
        ...
        "li_item"    : ".CheckLog",
        "param"      : ["time"],
        "ajax_url"   : "<?php  echo $this->createMobileUrl('tallstuchecklog', array('schoolid' => $schoolid,'op'=>'More_Data' ), true)?>",
