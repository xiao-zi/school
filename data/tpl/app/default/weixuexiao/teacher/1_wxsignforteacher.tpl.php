...
<section class="signin_section">
	...
	<span class="needSigninShow" id="signstart">签到准备中</span>
	<a href="javascript:;" id="signinShow" class="needSigninShow" style="display:none;" >开始签到</a>
...
<script>
	...
	function GetRange(lat,lon) {
        let lat1 = <?php  echo $school['lat'];?>; //本部
        let lon1 = <?php  echo $school['lng'];?>;
        let start = new qq.maps.LatLng(lat1, lon1);
        let end = new qq.maps.LatLng(lat, lon);
        let bet; //距离
		<?php  if(is_showgkk() || is_TestFz()) { ?>
        let this_range = <?php  echo $school['wxsignrange'];?> ? <?php  echo $school['wxsignrange'];?> : 1 ;
		<?php  } else { ?>
        let this_range =  1 ;
		<?php  } ?>
        ...
        qq.maps.convertor.translate(start, 3, function(res){
    		...
    		let latlng = res[0];
            //console.log(latlng);
            bet = Math.round(qq.maps.geometry.spherical.computeDistanceBetween(end, latlng) * 10) / 10;
            bet_range = bet;
			// console.log(is_showgkk())
			<?php  if(is_showgkk() || is_TestFz()) { ?>
				if (bet_range <= Number(this_range) || this_range <= 1 ){
					$("#school_range").text(bet_range + "米");
					$("#bet").val(bet_range);
					$('#signin2').show();
					$('#signin').show();
					$('#signin1').show();
                    ...
                    <?php  if(!empty($fxlocation['lat']) && !empty($fxlocation['lng']) ) { ?>
	...
	<?php  if(is_showgkk() || is_TestFz()) { ?>
	function GetRange_fx(end){
		//console.log("Infx");
		<?php  if(!empty($fxlocation['lat']) && !empty($fxlocation['lng']) ) { ?>
		let lat1_fx = <?php  echo $fxlocation['lat'];?>;
		let lon1_fx = <?php  echo $fxlocation['lng'];?>;
		<?php  } else { ?>
        let lat1_fx = 0;
        let lon1_fx = 0;
		<?php  } ?>
		let start_fx = new qq.maps.LatLng(lat1_fx, lon1_fx);
		let bet; //距离
		//转换经纬度（不同系之间的转换)
		qq.maps.convertor.translate(start_fx, 3, function(res){
			//取出经纬度并且赋值
			let latlng = res[0];
			bet = Math.round(qq.maps.geometry.spherical.computeDistanceBetween(end, latlng) * 10) / 10;
			if(bet_range > bet){
				bet_range = bet;
				$("#school_range").text(bet_range + "米【分校】");
			}else{
				$("#school_range").text(bet_range + "米");
			}
			$("#bet").val(bet_range);
