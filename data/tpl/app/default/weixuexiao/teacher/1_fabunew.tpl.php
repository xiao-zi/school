...
<div class="feedback_box">
    <?php  if(is_showgkk() && IsHasQx($tid_global,2000304,2,$schoolid)) { ?>
    <div class="blank"></div>
    <div class="feedback_title_box">
        <select class="feedback_title"  name="fb_tid" id="fb_tid">
            <option value="">请选择老师</option>
            <?php  if(is_array($teachers_list)) { foreach($teachers_list as $row) { ?>
            <option value="<?php  echo $row['id'];?>" tname="<?php  echo $row['tname'];?>"><?php  echo $row['tname'];?></option>
            <?php  } } ?>
        </select>
    </div>
    <?php  } ?>
    ...
    <div class="weui_cell_fts" style="text-align:left;margin-top: 10px;">
        <span>允许评论&nbsp;</span>
		...
		var choose_img_params = {
			...
			wxsdkSendData: function (imgServerid, videoServerid, videoTime, media_receiveid) {
				if (submit_wxsdkSendData) {
				    ...
				    fb_tid = $("#fb_tid").val();
                    fb_tname = $('#fb_tid option:selected') .attr("tname");//选中的值
					var data = {
						weid:"<?php  echo $weid;?>",
        ...
        var uploader = new AliyunUpload.Vod({
          ...
          onUploadstarted: function (uploadInfo) {
			if (!uploadInfo.videoId) {
				  var createUrl = "<?php  echo $this->createMobileUrl('bjqajax',array('op'=>'CreateAliUploadVideo','schoolid'=> $schoolid))?>" + "&title=<?php  echo $teachers['tname'];?>老师"  + "&filename=" + uploadInfo.file.name + "&description=班级通知"+"&tag=班级通知&coverurl="
