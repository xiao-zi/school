...
<div id="titlebar" class="header mainColor">
	...
	<div class="m">
		<span style="font-size: 18px">校园群发</span>   
	</div>
</div>    
...
<div class="feedback_box">
    ...
    <div class="feedback_title_box">
        ...
        <select id="master_select_box">
            ...
            <option value="school">@全体师生</option>
			<option value="alltea">@全体老师</option>
			<option value="allstu">@全体学生</option>
            <option value="send_class">指定班级</option>
            <option value="student">指定学生个人</option>
			<option value="staff_jsfz">指定老师组</option>
            <option value="staff">指定老师个人</option>
        </select>
    </div>
	<div class="blank"></div>
	<div class="feedback_title_box">
    ...
    <div class="weui_cell_ft">
        <span>同时发表至首页&nbsp;</span><input id="is_personal" class="weui_switch" type="checkbox" value="私密"/>
    </div>
...
setTimeout(function() {
	...
	$(function () {
		if (window.localStorage) {
			...
			var option_li_box2 = localStorage.getItem("option_li_box2");
			var option_li_box3 = localStorage.getItem("option_li_box3");
			var option_li_box1 = localStorage.getItem("option_li_box1");
			var option_li_box4 = localStorage.getItem("option_li_box4");
			if (option_li_box1 != "" && option_li_box1 != null && option_li_box1 != "null") {
				$('.option_li_box1').append(option_li_box1);
			}
			if (option_li_box2 != "" && option_li_box2 != null && option_li_box2 != "null") {
				$('.option_li_box2').append(option_li_box2);
			}
			if (option_li_box3 != "" && option_li_box3 != null && option_li_box3 != "null") {
				$('.option_li_box3').append(option_li_box3);
			}
			if (option_li_box4 != "" && option_li_box4 != null && option_li_box4 != "null") {
				$('.option_li_box4').append(option_li_box4);
			}
		}
		...
		$("#is_personal").on("change", function () {
            ...
            if (!ios_nav) {
                ...
                $("#master_select_box").on("change", function() {
                    ...
                    if ($(this).val() == "school" || $(this).val() == "alltea" || $(this).val() == "allstu") {
						...
						if ($(this).val() == "student") {
							...
							$(".option_li_box4").removeClass("show");
							$(".option_li_box3").removeClass("show");
							$(".option_li_box1").addClass("show");
							$(".weui_cell_ft").hide();
							$(".header").hide();
							getnotice_user('staff');
						} else if ($(this).val() == "staff_jsfz") {
							$(".option_li_box2").removeClass("show");
							$(".option_li_box1").removeClass("show");
							$(".option_li_box3").removeClass("show");
							$(".option_li_box4").addClass("show");
							$(".weui_cell_ft").hide();
							$(".header").hide();
							getnotice_user('staff_jsfz');
						}
                    }
			...
			$(".new_diary_list6").on("click",".option_list_ul3 li", function() {
				if ($(this).hasClass("jiantou")) {
					$(this).removeClass("jiantou");
					 $(this).addClass("jiantou2");
				}else{
			...
			$(".new_diary_list6").on("click",".option_list_ul1 .option_list_ul3 li", function(e) {
                e.stopPropagation();
                e.preventDefault();
                if ($(this).hasClass("check")) {
                    $(this).removeClass("check");
            ...
            $(".new_diary_list6").on("input propertychange",".seachBox", function() {
				...
				if ($("#master_select_box").val() == 'staff') {
					$(".sec_ul_box").removeClass("show");
                    $(".option_list_ul1 li").each(function() {
                        if ($(this).find(".name2").text().indexOf(input_text) >= 0) {
							$(this).addClass("show");
				...
				if ($("#master_select_box").val() == 'staff_jsfz') {
                    $(".option_list_ul li").each(function() {
                        if ($(this).children(".name").text().indexOf(input_text) >= 0) {
                            $(this).addClass("show");
                        } else {
            ...
            //确认按钮   staff
			$(".new_diary_list6").on("click",".sure_btn1", function() {
                //$("#feedback_title").attr("select_id",'').text('');
                var all_select_text = '';
                var all_select_id = '';
                $(".option_list_ul3 li.check").each(function() {
			...
			$(".new_diary_list6").on("click",".sure_btn4", function() {
                //$("#feedback_title").attr("select_id",'').text('');
                var all_select_text = '';
                var all_select_id = '';
                $(".option_list_ul_staff_jsfz li.check").each(function() {
                    all_select_text += $(this).children(".name").text() + ';';
                    all_select_id += $(this).attr("fz_id") + ';';
                });
                if (all_select_id) {
                    $("#feedback_title").attr("select_id", all_select_id).text(all_select_text);
			...
			$(".new_diary_list6").on("click",".sure_btn2", function() {
                var all_select_text = '';
                var all_select_id = '';
                $(".option_list_ul2 li.check").each(function() {
                    all_select_text += $(this).find(".name").text() + ';';
                    all_select_id += $(this).attr("u_id") + ';';
                });
                if (all_select_id != "") {
		...
		var choose_img_params = {
			...
			wxsdkSendData: function (imgServerid, videoServerid, videoTime, media_receiveid) {
				if (submit_wxsdkSendData) {
					...
					var url = "<?php  echo $this->createMobileUrl('dongtaiajax',array('op'=>'mfabu'))?>";
		...
		$.ajax({
			...
			success: function (data) {
				if ($.trim(data)) {
					...
					if(type == "staff"){
						$('.option_li_box1').empty();
						$('.option_li_box1').append(data);
					}
					if(type == "staff_jsfz"){
						$('.option_li_box4').empty();
						$('.option_li_box4').append(data);
					}
				}else{
		...
		$.ajax({
			...
			success: function (result) {
				...
				if (result.status == 1) {
					...
					ajax_stop_loading();					
					upload_pro(page,noticeid,groupid,total, this);
				} else {
	...
	function upload_pro(page, noticeid,groupid,total, self) {
		var prodata = {
			weid:"<?php  echo $weid;?>",
			schoolid : "<?php  echo $schoolid;?>",
			tname:"<?php  echo $teacher['tname'];?>",
            ...
            userdatas:$('#feedback_title').attr('select_id'),
			total:total,
			page:page
		}					
		$.ajax({
			url: "<?php  echo $this->createMobileUrl('dongtaiajax', array('schoolid' => $schoolid,'op' => 'mnotpro'))?>",
			data: prodata,
			type: "POST",
			dataType: "json",
			success: function (result) {
				...
				if (result.status == 2) {
					...
					location.href = "<?php  echo $this->createMobileUrl('mnoticelist', array('schoolid' => $schoolid))?>";					
				}				
			},
			error: function () {
				//提交后 隐藏加载层
        ...
        var uploader = new AliyunUpload.Vod({
          ...
          onUploadstarted: function (uploadInfo) {
			if (!uploadInfo.videoId) {
				  var createUrl = "<?php  echo $this->createMobileUrl('bjqajax',array('op'=>'CreateAliUploadVideo','schoolid'=> $schoolid))?>" + "&title=<?php  echo $teachers['tname'];?>老师"  + "&filename=" + uploadInfo.file.name + "&description=校园通知"+"&tag=校园通知&coverurl="
