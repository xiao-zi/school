...
<link type="text/css" rel="stylesheet" href="<?php echo OSSURL;?>public/mobile/css/mBbkq.css?v=5.11119" />
...
<style>
.babyBrief .r .name i{height:20px;width:20px;font-size:10px;float:left;background:url(<?php echo OSSURL;?>public/mobile/img/<?php  if($student['sex'] ==2) { ?>ico_girl.png<?php  } else { ?>ico_boy.png<?php  } ?>) no-repeat center;background-size: 20px 20px;}
.babyBrief .r .info i{height:20px;width:20px;float:left;background:url(<?php echo OSSURL;?>public/mobile/img/ico_clock_small.png) no-repeat center;margin-right:5px;background-size: 20px 20px;margin-top:20px;}
</style>
</head>
<body>
<input id="logid" value="<?php  echo $logid;?>" type="hidden">
...
<div class="_header"></div>
	<div class="babyBrief">
		<div class="l">
			<div class="img">
				<img alt="" src="<?php  if(!empty($student['icon'])) { ?><?php  echo tomedia($student['icon'])?><?php  } else { ?><?php  echo tomedia($school['spic'])?><?php  } ?>" />
			</div>
		</div>
		<div class="r">
			<div class="name">
			...
			<div class="info">
				<table>
					<tr>
						...
						<td class="r">
								<span><?php  if(!empty($log)) { ?>点击下面图标查看详情↓↓<?php  } else { ?><?php  echo $student['s_name'];?>今天尚未打卡哦1!<?php  } ?></span>
						</td>
					</tr>
				</table>
			</div>
	...
	<?php  if(!empty($log)) { ?>
		...
		<?php  if(is_array($log)) { foreach($log as $item) { ?>
			<div class="interval interval">
				<div class="paxy" onclick="viewPaxy(<?php  echo $item['id'];?>,this)" style="margin-top:50px;">
					 <div class="div-time"><?php  echo date('H:i',$item['createtime'])?></div>
					 <div class="div-img">
						<img src="<?php echo OSSURL;?>public/mobile/img/gh_xh_wating.gif" />
					 </div>
				</div>
			</div>							
		<?php  } } ?>
	...
	<div class="viewPushBox">
		<div class="pushImg parent">
		   <a onclick="wxImageShow(this)" id="pic">
				<img src="<?php  if(!empty($student['icon'])) { ?><?php  echo tomedia($student['icon'])?><?php  } else { ?><?php  echo tomedia($school['spic'])?><?php  } ?>" >
		   </a>
		...
		$.ajax({
			...
			success: function(datas){
				...
				var msg = result.data.msg;
					...
					var image2 = result.data.pic2;
					var mac = result.data.mactype;
					if(image===""){
						 image = '<?php  echo $student['icon'];?>';
					}
					setPaxyData(image,image2,result.data.cardid,bodytemp,result.data.pard,result.data.type,result.data.leixing,result.data.macname,mac);
			}
		});
}

function setPaxyData(image,image2,cardid,bodytemp,pard,type,leixing,macname,mac){
	...
	if (images2 != "" || images2 != null){
		var html = '<img alt="" src="' + images + '" />' +
				   '<img alt="" src="' + images2 + '" />';
				   ;
		$('#pic').append(html);
	}else{
