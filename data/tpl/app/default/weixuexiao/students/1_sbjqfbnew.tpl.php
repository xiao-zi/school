		...
		if (window.localStorage) {
		 ...
		 // 添加链接事件
			$(".add_link_btn").on("click",function(){
				$(".add_link_box").show(200);
			})
			$("#add_link_btn_cancel").on("click",function(){
				$(".add_link_box").hide(200);
			})
		...
		var choose_img_params = {
			...
			wxsdkSendData: function (imgServerid, videoServerid, videoTime, media_receiveid) {
				if (submit_wxsdkSendData) {
					...
					var url = "<?php  echo $this->createMobileUrl('AppCircleAjax',array('op'=>'studentPublishCircle'))?>";
          ...
          onUploadstarted: function (uploadInfo) {
			if (!uploadInfo.videoId) {
				  ...
				  <?php  } else { ?>
					...
					let tag = "校园圈";
				  <?php  } ?>
				  var createUrl = "<?php  echo $this->createMobileUrl('bjqajax',array('op'=>'CreateAliUploadVideo','schoolid'=> $schoolid))?>" + "&title=<?php  echo $students['s_name'];?><?php  echo $shenfen;?>"  + "&filename=" + uploadInfo.file.name + "&description="+description+"&tag="+tag+"&coverurl="
