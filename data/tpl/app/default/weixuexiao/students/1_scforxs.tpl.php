...
<div class="header mainColor" id="titlebar">
	<div class="l">
		<a class="backOff" style="background:url(<?php echo OSSURL;?>public/mobile/img/ic_arrow_left_48px_white.svg) no-repeat;background-size: 55% 55%;background-position: 50%;" href="<?php  echo $this->createMobileUrl('sclistforxs', array('schoolid' => $schoolid), true)?>"></a>
	</div>
	<div class="m">
...
<div class="manual_main1">
    <div class="manual_visible_box">
        <div class="manual_til_img">
            <?php  if($_GPC['type'] == 'school') { ?><img src="<?php  echo tomedia($toppic['top1'])?>" /><?php  } ?>
        </div>
        <div class="manual_main_box2">
            ...
            <?php  if($_GPC['op'] =='edite') { ?>
				...
				<div class="parent_evaluation_box">
					<textarea placeholder="说说学生在家表现" id="eval_text" ><?php  echo $mypl['jzword'];?></textarea>
			...
			<?php  } else if($_GPC['op'] =='check') { ?>
				...
				<?php  } } ?>
				<div class="blank"></div>	
				<?php  if(!$mypl['jzword']) { ?><a href="<?php  echo $this->createMobileUrl('scforxs', array('schoolid' => $schoolid,'sid' => $sid,'scid' => $scid,'setid' => $setid, 'op' => 'edite'), true)?>" class="white_btn2 edit-btn" id="add_comment" >我也说两句</a><?php  } ?>				
			<?php  } else { ?>
				<ul class="manual_baby_situation_ul">
					<?php  if(is_array($list)) { foreach($list as $row) { ?>
						<li>
...
<?php  if($_GPC['op'] =='check') { ?>
    <div class="bottom_control_row">
		 <?php  if($_GPC['type'] =='home') { ?>
			<div class="prev_btn2"><a href="<?php  echo $this->createMobileUrl('scforxs', array('schoolid' => $schoolid,'sid' => $sid,'scid' => $scid,'setid' => $setid, 'op' => 'check', 'type' => 'school'), true)?>">在校情况</a></div>
		 <?php  } ?>	
		 <?php  if($_GPC['type'] =='school' || !$_GPC['type']) { ?>
			<div class="prev_btn2"><a href="<?php  echo $this->createMobileUrl('scforxs', array('schoolid' => $schoolid,'sid' => $sid,'scid' => $scid,'setid' => $setid, 'op' => 'check', 'type' => 'home'), true)?>">在家情况</a></div>		 
		 <?php  } ?>
        <div class="next_btn blue_bg"><a href="<?php  echo $this->createMobileUrl('scforxs', array('schoolid' => $schoolid,'sid' => $sid,'scid' => $scid,'setid' => $setid, 'op' => 'edite'), true)?>">修改家情况</a></div>
    </div>
<?php  } else if($_GPC['op'] =='edite') { ?>
    <div class="bottom_control_row">
            <div class="prev_btn1" type="prev"><a href="javascript:void(0);">保存</a></div>
...
$(".prev_btn").on("click", function () {
	...
	$.ajax({
		url: "<?php  echo $this->createMobileUrl('scforxs', array('schoolid' => $schoolid,'op' => 'savedata'), true)?>",
